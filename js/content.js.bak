...
async function downloadChatGPTConversationAsHtml() {
  /* ========= helpers ========= */
  ...
  clonedRoot.querySelectorAll("*").forEach((el) => {
    ...
    if (style.color?.startsWith("rgb")) el.style.color = style.color;
...
pre, code {
  ...
  white-space:pre; /* 保持程式碼不換行，維持可讀性 */
  background:#f6f8fa !important;
  color:#111 !important;
  padding:12px;
  border-radius:6px;
  margin:12px 0;
  overflow-x:auto; /* 超出寬度時出現卷軸 */
  line-height:1.5;
  max-width: 100%; /* 確保程式碼區塊不會撐爆頁面 */
  font-size:13px;
  overflow-y: auto;
}
  ...
  function generateButtons() {
    ...
    if(supportChatGPT){
      ...
      downloadHtmlButton.addEventListener("click", () => {
        downloadChatGPTConversationAsHtml();
      });
